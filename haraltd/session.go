	...
	if err := s.startListener(ctx, cfg.SocketPath); err != nil {
		return nil, platform,
			fault.Wrap(
				...
				fctx.With(context.Background(), "error_at", "listener-haraltd"),
				ftag.With(ftag.Internal),
	...
	if err != nil {
		return nil, platform,
			fault.Wrap(
				...
				ftag.With(ftag.Internal),
				fmsg.With("Cannot get advertised features from haraltd"),
			)
	}
...
func (s *HaraltdSession) Stop() error {
...
func (s *HaraltdSession) Adapters() ([]bluetooth.AdapterData, error) {
	return s.store.Adapters()
}

// Adapter returns a function call interface to invoke adapter related functions.
func (s *HaraltdSession) Adapter(address bluetooth.AdapterAddress) bluetooth.Adapter {
	return &adapter{s, address}
}

// Device returns a function call interface to invoke device related functions.
func (s *HaraltdSession) Device(address bluetooth.DeviceAddress) bluetooth.Device {
...
func (s *HaraltdSession) Network(bluetooth.DeviceAddress) bluetooth.Network {
	return &network{}
}

// MediaPlayer returns a function call interface to invoke media player/control
// related functions on a device.
func (s *HaraltdSession) MediaPlayer(bluetooth.DeviceAddress) bluetooth.MediaPlayer {
	return &mediaPlayer{}
}
...
func (s *HaraltdSession) emptyAdapter() *adapter {
	return &adapter{}
}
...
func (s *HaraltdSession) emptyDevice() *device {
	return &device{}
}

// refreshStore refreshes the global session store with adapter and device objects
// that are retrieved from the haraltd daemon.
func (s *HaraltdSession) refreshStore() error {
...
func (s *HaraltdSession) startListener(ctx context.Context, socketpath string) error {
...
func (s *HaraltdSession) listen(ctx context.Context) {
...
func (s *HaraltdSession) handleListenerEvent(ev events.ServerEvent) {
	switch ev.EventID {
...
func (s *HaraltdSession) handleListenerError(err error, stop bool) {
	if err != nil {
		bluetooth.ErrorEvents().PublishAdded(wrapError(err))
	}
...
func (s *HaraltdSession) executor(params []string) (chan commands.CommandResponse, error) {
	if s.sessionClosed.Load() {
...
func (s *HaraltdSession) reset(isClosed bool) context.Context {
	s.Lock()
	defer s.Unlock()
